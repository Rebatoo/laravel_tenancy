...
<body class="bg-gray-50">
        ...
        <div class="max-w-md w-full space-y-8">
            ...
            <form class="mt-8 space-y-6" action="{{ route('tenant.register') }}" method="POST">
                ...
                @if(session('google_data'))
                    <div class="mb-4">
                        <p class="text-green-600">You are registering with Google. Please complete the form below.</p>
                    </div>
                @else
                    <div class="mb-4">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const googleData = @json(session('google_tenant_data'));
            if (googleData) {
                document.getElementById('name').value = googleData.name;
